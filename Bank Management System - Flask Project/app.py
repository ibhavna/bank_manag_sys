...
c = conn.cursor()
c.execute('CREATE TABLE IF NOT EXISTS banking (ID INTEGER PRIMARY KEY AUTOINCREMENT, name VARCHAR(256), pin VARCHAR(256) ,  balance INTEGER, loan INTEGER, interest INTEGER, account_no INTEGER UNIQUE)')
conn.close()
app = Flask('')
app.secret_key = "abc"
...
def hello_world():
    ...
    try:
            ...
            if request.method == "GET":
               return render_template("index.html")
                ...
                if mode == "register":
                    ...
                    c = conn.cursor()
                    try:
                      c.execute('INSERT INTO banking (name, pin, balance, loan, interest, account_no ) VALUES ("{}", "{}", {}, {}, {},{})'.format(username, password, 0,0,0,int(account_no)))
                      conn.commit()
                    except ValueError:
                        return render_template("index.html", error="PLEASE ENTER A PROPER VALUE!")
                    except Exception as e:
                        print(e)
                        return render_template("index.html", error="ACCOUNT NUMBER ALREADY EXISTS!")
                    row=c.execute("SELECT * FROM banking WHERE name='{}' AND pin='{}' AND balance={} AND loan={} AND interest={} AND account_no={}".format(username, password,0,0,0, int(account_no)))
                    target=row.fetchone()
        ...
        print(e)
        return render_template("index.html",error = "PLEASE CHECK YOUR CREDENTIALS!")
...
def delete():
    if session["loggedin"] == "yes":
        conn = sqlite3.connect('database.db')
        c = conn.cursor()
        row = c.execute("DELETE FROM banking WHERE id = {}".format(session['id']))
        target = row.fetchone()
        conn.commit()
        conn.close()
        session.pop('username', None)
